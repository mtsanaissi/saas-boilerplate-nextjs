...
type MagicSearchParams = {
  error?: string;
  success?: string;
};

interface MagicPageProps {
  searchParams?: Promise<MagicSearchParams>;
...
export default async function MagicPage({
  ...
  const [{ locale }, resolvedSearchParams] = await Promise.all([
    params,
    searchParams ?? Promise.resolve<MagicSearchParams>({}),
  ]);

  const [t, tErrors] = await Promise.all([
    getTranslations({ locale, namespace: "auth.magic" }),
  ...
  const formErrorId = errorMessage ? "auth-magic-error" : undefined;
